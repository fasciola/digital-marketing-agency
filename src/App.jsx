...
const images = {
  hero: '/images/ChatGPT Image Jun 16, 2026, 05_06_36 PM (1).png',
  website: '/images/ChatGPT Image Jun 16, 2026, 05_06_37 PM (2).png',
  analytics: '/images/ChatGPT Image Jun 16, 2026, 05_06_38 PM (5).png',
  portfolio: '/images/ChatGPT Image Jun 16, 2026, 05_06_39 PM (6).png',
  flatlay: '/images/ChatGPT Image Jun 16, 2026, 05_06_37 PM (3).png',
  phone: '/images/ChatGPT Image Jun 16, 2026, 05_06_38 PM (4).png',
};
...
const portfolio = [
  { title: 'Business Growth Website', category: 'Business Websites', meta: 'Website / Lead Gen / 2026', image: images.website },
  { title: 'Premium Shopfront', category: 'E-commerce', meta: 'Store / Campaigns / 2026', image: images.analytics },
  { title: 'Conversion Landing Page', category: 'Landing Pages', meta: 'Ads / WhatsApp / 2026', image: images.portfolio },
  { title: 'Social Launch Campaign', category: 'Social Media Campaigns', meta: 'Content / Growth / 2026', image: images.flatlay },
  { title: 'Luxury Brand Identity', category: 'Branding Projects', meta: 'Identity / Positioning / 2026', image: images.phone },
];
const processSteps = [
...
function Hero() {
  return (
    <section id="top" className="beauty-hero image-hero">
      <img className="hero-bg-image" src={images.hero} alt="Premium digital marketing workspace with phone, analytics, and brand materials" loading="eager" />
      ...
      <div className="bottom-video-card image-video-card"><div className="play-button">▶</div><div><strong>Watch growth system</strong><span>Websites · SEO · Ads · Content</span></div></div>
    </section>
  );
}

function Intro() { return <section id="about" className="intro-grid container image-intro"><article className="panel intro-large"><Label>About Digital Motion</Label><h2>Marketing systems that reflect who you are.</h2><p>Every detail matters. From concept to conversion, we create digital experiences that are elegant, functional, and built to generate enquiries.</p></article><article className="panel intro-card image-card"><img src={images.website} alt="Laptop and phone showing premium website design" loading="lazy" /><div><h3>Thoughtful Design</h3><p>Intentional layouts that enhance the way customers understand your offer.</p><a href="#services">Learn more <ArrowRight size={14} /></a></div></article><article className="panel intro-card image-card"><img src={images.analytics} alt="Tablet analytics dashboard for marketing performance" loading="lazy" /><div><h3>Timeless Strategy</h3><p>Clear marketing systems that balance brand presence and measurable growth.</p><a href="#process">Learn more <ArrowRight size={14} /></a></div></article></section>; }
function Services() { return <section id="services" className="section video-section"><VideoBackground name="services-bg" /><div className="container"><div className="section-head"><Label>Services</Label><h2>Designed to make your business look trusted and generate enquiries.</h2></div><div className="services-grid">{services.map((service, index) => { const Icon = service.icon; const serviceImages = [images.website, images.phone, images.analytics, images.flatlay]; return <article className="panel service-card" key={service.title}><Icon /><h3>{service.title}</h3><p>{service.text}</p>{index < 4 && <img className="service-thumb" src={serviceImages[index]} alt="Marketing service visual" loading="lazy" />}</article>; })}</div></div></section>; }
function Portfolio() { const [active, setActive] = useState(0); const item = portfolio[active]; return <section id="work" className="projects-section container"><div className="projects-copy panel"><Label>Selected Projects</Label><ul className="project-list large">{portfolio.map((project, index) => <li className={index === active ? 'active' : ''} key={project.title} onClick={() => setActive(index)}><span>{project.category}</span><strong>{project.title}</strong></li>)}</ul></div><div className="project-preview panel image-panel real-project-preview"><img src={item.image} alt={`${item.title} preview`} loading="lazy" /><div className="preview-note"><strong>{item.title}</strong><span>{item.meta}</span><a href={whatsappUrl} target="_blank" rel="noreferrer">Start Similar Project <ArrowRight size={14} /></a></div><div className="preview-meta"><span>Size<br />Custom</span><span>Location<br />UAE</span><span>Year<br />2026</span><span>Type<br />{item.category}</span></div></div></section>; }
...
function FinalCta() { return <section id="contact" className="final-cta container"><div className="panel cta-copy"><Label>Our Upcoming Concept</Label><h2>Ready to grow your business online?</h2><p>Let’s build a website, campaign, and marketing system that helps your business attract more customers.</p><a className="dark-btn" href={whatsappUrl} target="_blank" rel="noreferrer">Chat With Us on WhatsApp <ArrowRight size={16} /></a></div><div className="panel cta-image real-cta-image"><img src={images.portfolio} alt="Premium marketing workspace with laptop and phone" loading="lazy" /><span>WhatsApp Lead System</span></div></section>; }
